...
function selectOngoingEvent(eventId) {
  document.getElementById('content').innerHTML = `
    ...
    <div class="action-buttons">
      <button class="register-button" onclick="registerParticipant(${eventId})">Registrovat účastníka</button>
      <button class="center-button add-drink-button" onclick="showAddDrinkForm(${eventId})">Přidat drink</button>
      <button class="end-event-button" onclick="endEvent(${eventId})">Ukončit akci</button>
    </div>
    <div class="back-container"><button class="back-button" onclick="showOngoingEvents()">Zpět</button></div>
  `;
}
...
function showAddDrinkForm(eventId) {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <form id="addDrinkForm">
      <label for="participantId">Účastník:</label>
    ...
    <div class="back-container"><button class="back-button" onclick="selectOngoingEvent(${eventId})">Zpět</button></div>
  `;
...
function showEventOptions(eventId) {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <label for="participantFilter">Zvolit účastníka:</label>
    <select id="participantFilter" name="participantFilter">
      <option value="all">Všichni</option>
      ${participantOptions}
    </select>
    <div class="option-buttons">
      <button onclick="showEventDetails(${event.id}, 'timeline')">Průběžná tabulka v čase</button>
      <button onclick="showEventDetails(${event.id}, 'summary')">Celkové výsledky</button>
      <button onclick="showMVP(${event.id})">MVP</button>
    </div>
    <div class="back-container"><button class="back-button" onclick="showEndedEvents()">Zpět</button></div>
  `;
}
...
function showEventDetails(eventId, viewType) {
  ...
  if (event) {
    ...
    document.getElementById('content').innerHTML = `
      ...
      <div class="back-container"><button class="back-button" onclick="showEventOptions(${eventId})">Zpět</button></div>
    `;
  }
}
  ...
  if (event) {
    ...
    const mvpDetails = mvpList.map(mvp => `
      <tr>
        ...
        <td>${mvp.maxCount}</td>
        <td>${mvp.maxPrice.toFixed(2)} Kč</td>
      </tr>
    `).join('');
    ...
    document.getElementById('content').innerHTML = `
      ...
      <div class="summary-table">
        <h3>Souhrn</h3>
        <p>Počet drinků celkem: ${totalDrinks}</p>
        <p>Objem celkem: ${totalVolume.toFixed(2)} l</p>
        <p>Cena celkem: ${totalPrice.toFixed(2)} Kč</p>
      </div>
      <div class="back-container"><button class="back-button" onclick="showEventOptions(${eventId})">Zpět</button></div>
    `;
  }
